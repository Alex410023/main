...
void DirectedGraph::draw(QPainter& painter, const int activeVertex) {
    qreal radius = 30;
    ...
    for (const QPointF& vertex : vertices) {
        if (currentIndex == activeVertex) {
            QBrush activeVertexBrush(Qt::green, Qt::SolidPattern);
            painter.setBrush(activeVertexBrush);
        } else {
            painter.setBrush(vertexBrush);
        }
