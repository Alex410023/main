...
TInterface::TInterface(QWidget *parent)
    ...
    connect(detBtn, &QPushButton::clicked, this, &TInterface::detButtonClicked);

    transposeBtn = new QPushButton("Печать транспонированной матрицы");
    connect(transposeBtn, &QPushButton::clicked, this, &TInterface::transposeButtonClicked);

    rankBtn = new QPushButton("Печать ранга");
    connect(rankBtn, &QPushButton::clicked, this, &TInterface::rankButtonClicked);

    printBtn = new QPushButton("Печать матрицы");
    connect(printBtn, &QPushButton::clicked, this, &TInterface::printButtonClicked);
    ...
    if (matrixSizeInput->text().toInt() > 0) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                matrixLayout->addWidget(createRationalEditor(new TRational()), i, j);
            }
        }
...
void TInterface::detButtonClicked() {
    int size = matrixSizeInput->text().toInt();
            ...
            if (item) {
                ...
                if (QLineEdit* numeratorEditor = qobject_cast<QLineEdit*>(widget->findChild<QLineEdit*>("numeratorEditor"))) {
                    ...
                    if (QLineEdit* denominatorEditor = qobject_cast<QLineEdit*>(widget->findChild<QLineEdit*>("denominatorEditor"))) {
                        ...
                        squareMatrix.setAt(i, j, TRational(numerator, denominator));
                    }
                }
            }
        }
    ...
    str += "Determinant: ";
    str << squareMatrix.calculateDeterminant();

    output->setText(str);
}

void TInterface::transposeButtonClicked() {
    int size = matrixSizeInput->text().toInt();
    SquareMatrix squareMatrix(size);
    QString str;
    ...
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            QLayoutItem *item = matrixLayout->itemAtPosition(j, i);
            if (item) {
                QWidget* widget = item->widget();
                if (QLineEdit* numeratorEditor = qobject_cast<QLineEdit*>(widget->findChild<QLineEdit*>("numeratorEditor"))) {
                    int numerator = numeratorEditor->text().toInt();
                    if (QLineEdit* denominatorEditor = qobject_cast<QLineEdit*>(widget->findChild<QLineEdit*>("denominatorEditor"))) {
                        int denominator = denominatorEditor->text().toInt();
                        str << TRational(numerator, denominator);
                        str += "\t\t";
                    }
                }
            }
...
void TInterface::rankButtonClicked() {
    int size = matrixSizeInput->text().toInt();
    SquareMatrix squareMatrix(size);
    QString str;
    ...
    str += "Rank: ";
    str += QString::number(squareMatrix.calculateRank());

    output->setText(str);
}

void TInterface::printButtonClicked() {
    int size = matrixSizeInput->text().toInt();
    SquareMatrix squareMatrix(size);
    QString str;
...
QWidget* TInterface::createRationalEditor(TRational* rational) {
    ...
    numeratorEditor->setText(QString::number(rational->getNum()));
    ...
    denominatorEditor->setText(QString::number(rational->getDenom()));
    editorLayout->addWidget(denominatorEditor);

    connect(numeratorEditor, &QLineEdit::textEdited, this, [rational, numeratorEditor]() {
        rational->setNum(numeratorEditor->text().toInt());
    });

    connect(denominatorEditor, &QLineEdit::textEdited, this, [rational, denominatorEditor]() {
        rational->setDenom(denominatorEditor->text().toInt());
    });
